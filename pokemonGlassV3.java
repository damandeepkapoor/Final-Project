...
public class pokemonGlassV3 extends Applet implements ActionListener, KeyListener
{
    AudioClip music;
    ...
    int pokeHp[] = {55, 44, 53}; //starting hp
    int lvl = 0; //lvl counter (changes once you battle)
    int hp = 100; //hp counter

    String wildPokeName[] = {"Starly", "Bidoof", "Kricketot", "Shinx", "Abra", "Zubat", "Budew", "Geodude", "Gligar", "Ralts", "Riolu", "Cranidos", "Machop", "Pachirisu", "Heracross", "Drifloon", "Happiny", "Chatot", "Pichu", "Pikachu", "Gible", "Croagunk", "Duskull", "Swablu", "Togepi", "Moltres", "Articuno", "Zapdos", "Mewtwo", "Mew", "Uxie", "Mesprit", "Azelf", "Dialga", "Giratina", "Palkia", "Mega Lucario", "Rotom", "Manaphy", "Shiny Lugia", "Shiny Gyarados", "Shiny Charzard", "Shiny Articuno"}; //wild pokemons you can catch
	...
	/*
		...
		JFrame fP = new JFrame ("EXP");
		fP.setDefaultCloseOperation (JFrame.EXIT_ON_CLOSE);
		Container content2 = fP.getContentPane ();
		JProgressBar eP = new JProgressBar ();
		eP.setValue (exp);
		...
		JFrame level = new JFrame ("Level");
		level.setDefaultCloseOperation (JFrame.EXIT_ON_CLOSE);
		Container content3 = level.getContentPane ();
		JLabel lvlStarter = new JLabel ("Your" + pokeName1 [uPoke] + " is level:" + pokeLvl [uPoke]);
		...
		*/
	uP = new JProgressBar (0, 0, hp); //hp of wild pokemon
	uP.setValue (hp);
	uP.setMaximum (hp);
	uP.setStringPainted (true);
	uP.setPreferredSize (new Dimension (150, 20));
	uP.setForeground (Color.blue);

	eP = new JProgressBar (0, 0, reqExp); //hp of wild pokemon
	eP.setValue (exp);
	eP.setMaximum (reqExp);
	eP.setStringPainted (true);
	eP.setPreferredSize (new Dimension (100, 15));
	eP.setForeground (Color.blue);

	u.add (uP);
	u.add (eP);

	JButton att1 = new JButton ("Tackle"); //attack1
	...
	JButton att2 = new JButton ("Double Slap"); //attack2
	...
	JButton att3 = new JButton ("Hidden Power"); //attack3
	...
	if (exp >= reqExp)
	{
	    ...
	    {
		pokeName1 [uPoke].equals ("Monferno");
	    ...
	    {
		pokeName1 [uPoke].equalsIgnoreCase ("Empoleon");
		JOptionPane.showMessageDialog (null, "Your pokemon evolved to " + pokeName1 [uPoke], "Evolving", JOptionPane.INFORMATION_MESSAGE);
		i = 18;
	    }
	...
	if (pics [guyy] [guyx].equals ("201") && screen == 1)
	{
	    screen = 2;
	    guyy = 38;
	    guyx = 14;
	...
	else if (pics [guyy] [guyx].equals ("sgTown") && screen == 2)
	{
	    screen = 4;
	    guyy = 8;
	    guyx = 2;
	...
	else if (pics [guyy] [guyx].equals ("201") && screen == 4)
	{
	    screen = 2;
	    guyy = 8;
	    guyx = 48;
	...
	else if (pics [guyy] [guyx].equals ("pcCarpet"))
	{
	    screen = 4;
	    guyy = 24;
	    guyx = 18;
	...
	else if (pics [guyy] [guyx].equals ("mCarpet"))
	{
	    screen = 4;
	    guyy = 25;
	    guyx = 28;
	...
	else if (pics [guyy] [guyx].equals ("sgTown") && screen == 7)
	{
	    screen = 4;
	    guyy = 3;
	    guyx = 44;
	...
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("grass1"))
	{
	...
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("cFloor"))

	    {
		counter = (int) (Math.random () * 43) + 29;
		cHp = wildHp [counter];
		cP.setMaximum (cHp);
		cdLayout.show (p_card, "10");
		move++;
	...
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("grass2"))
	{
	...
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("grass3"))
	{
	...
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("grass4"))
	{
	    counter = (int) (Math.random () * 20) + 15;
	    cHp = wildHp [counter];
	    cP.setMaximum (cHp);
	    cdLayout.show (p_card, "10");
	    move++;
	}
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("grass5"))
	{
	    counter = (int) (Math.random () * 25) + 20;
	    cHp = wildHp [counter];
	    cP.setMaximum (cHp);
	    cdLayout.show (p_card, "10");
	    move++;
	}
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("grass6"))
	{
	    counter = (int) (Math.random () * 29) + 25;
	    cHp = wildHp [counter];
	    cP.setMaximum (cHp);
	    cdLayout.show (p_card, "10");
	    move++;
	...
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("ground"))  //step on tall grass and fight starts
	{
	    counter = (int) (Math.random () * 29) + 0;
	    cHp = wildHp [counter];
	    cP.setMaximum (cHp);
	...
	else if (move % 10 == 0 && pics [guyy] [guyx].equals ("cFloor") || pics [guyy] [guyx].equals ("grass1") || pics [guyy] [guyx].equals ("grass2") || pics [guyy] [guyx].equals ("grass3") || pics [guyy] [guyx].equals ("grass4") || pics [guyy] [guyx].equals ("grass5") || pics [guyy] [guyx].equals ("grass6") || pics [guyy] [guyx].equals ("grass7"))  //step on tall grass and fight starts
	{
	    if (pics [guyy] [guyx].equals ("grass1"))
		counter = (int) (Math.random () * 5) + 0;
	    else if (pics [guyy] [guyx].equals ("grass2"))
		counter = (int) (Math.random () * 10) + 5;
	    else if (pics [guyy] [guyx].equals ("grass3"))
		counter = (int) (Math.random () * 15) + 10;
	    else if (pics [guyy] [guyx].equals ("grass4"))
		counter = (int) (Math.random () * 20) + 15;
	    else if (pics [guyy] [guyx].equals ("grass5"))
		counter = (int) (Math.random () * 25) + 20;
	    else if (pics [guyy] [guyx].equals ("grass6"))
		counter = (int) (Math.random () * 29) + 25;
	    else if (pics [guyy] [guyx].equals ("grass7"))
		counter = (int) (Math.random () * 29) + 0;
	    else if (pics [guyy] [guyx].equals ("cFloor"))
		counter = (int) (Math.random () * 43) + 29;
	    cHp = wildHp [counter];
	    cP.setMaximum (cHp);
	    cdLayout.show (p_card, "10");
	...
	else if (pics [guyy] [guyx].equals ("talk1"))
	{
	...
	else if (pics [guyy] [guyx].equals ("talk2"))
	{
		...
		else
		{
		    ...
		    max = money / 200;
		    quantity = Integer.parseInt (JOptionPane.showInputDialog (null, "The max Pokeballs you can buy is " + max + ". How many do you want to buy? ", "PokeMart", JOptionPane.PLAIN_MESSAGE));
	...
	else if (pics [guyy] [guyx].equals ("talk3"))
	{
		...
		else
		{
		    ...
		    while (tC >= money)
			pQuantity = Integer.parseInt (JOptionPane.showInputDialog (null, "Insufficiant Funds. You can only buy " + max + " Potions. How many do you want to buy?", "PokeMart", JOptionPane.PLAIN_MESSAGE));
		    PotionQ += pQuantity;
		    money -= tC;
		}
	    }
	...
	else if (e.getActionCommand ().equals ("storyline"))
	    storyline ();
	...
	else if (e.getActionCommand ().equals ("att1"))
	{
	...
	else if (e.getActionCommand ().equals ("catching"))
	{
	    PokeballQ--;
	    if (catchMove % 2 == 0)
	    {
    ...
    {
	java.net.URL imgURL = pokemonGlassV3.class.getResource (path);
	if (imgURL != null)
